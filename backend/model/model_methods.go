...
func (u *User) IsForbidden() bool {
	...
	// 判断禁言时间
	return u.ForbiddenEndTime > time.Now().UnixNano()/1e6
}
...
func (u *User) InObservationPeriod(observeSeconds int) bool {
	...
	return time.Unix(0, u.CreateTime*int64(time.Millisecond)).Add(time.Second * time.Duration(observeSeconds)).After(time.Now())
}
