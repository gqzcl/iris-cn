...
func CopyImage(url string) (string, error) {
	...
	u2 := simple.ParseUrl(conf.Instance.BaseURL).GetURL()
	// 本站host，不下载
	if u1.Host == u2.Host {
		return url, nil
	}
...
func IsEnabledOss() bool {
	enable := conf.Instance.Uploader.Enable
...
func IsOssImageUrl(url string) bool {
	host := simple.ParseUrl(conf.Instance.Uploader.AliyunOss.Host).GetURL().Host
