...
func (s *topicService) GenerateRss() {
	...
	for _, topic := range topics {
		...
		item := &feeds.Item{
			...
			Description: auth.GetMarkdownSummary(topic.Content),
	...
	if err != nil {
		...
		_ = simple.WriteString(path.Join(conf.Instance.StaticPath, "topic_atom.xml"), atom, false)
	}
	...
	if err != nil {
		...
		_ = simple.WriteString(path.Join(conf.Instance.StaticPath, "topic_rss.xml"), rss, false)
	}
}
